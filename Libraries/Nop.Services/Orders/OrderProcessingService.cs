    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainter PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.Customer == null)
                ...
                throw new ArgumentException("客户未设定");
            ...
            if (details.Customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                ...
                throw new NopException("不允许匿名结帐");
            ...
            if (details.BillingAddress.Country != null && !details.BillingAddress.Country.AllowsBilling)
                ...
                throw new NopException(string.Format("国家： '{0}'，不允许进行结算", details.BillingAddress.Country.Name));
            ...
            if (details.Cart.RequiresShipping())
            {
                ...
                else
                {
                    if (details.Customer.ShippingAddress == null)
                        ...
                        throw new NopException("送货地址不提供");
                    ...
                    if (details.ShippingAddress.Country != null && !details.ShippingAddress.Country.AllowsShipping)
                        ...
                        throw new NopException(string.Format("国家： '{0}' 不允许运送", details.ShippingAddress.Country.Name));
                }
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                ...
                throw new NopException("运费总计无法计算");
            ...
            if (!orderTotal.HasValue)
                ...
                throw new NopException("无法计算订单总额");
        ...
        protected virtual PlaceOrderContainter PrepareRecurringOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.InitialOrder == null)
                ...
                throw new ArgumentException("初始订单未设定为定期付款");
            ...
            if (details.InitialOrder.BillingAddress == null)
                ...
                throw new NopException("帐单地址不可用");
            ...
            if (details.InitialOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                if (!details.PickUpInStore)
                {
                    if (details.InitialOrder.ShippingAddress == null)
                        ...
                        throw new NopException("送货地址不可用");
        ...
        protected virtual void SendNotificationsAndSaveNotes(Order order)
        {
            ...
            if (_workContext.OriginalCustomerIfImpersonated != null)
                ...
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("订单，店主('{0}'. ID = {1}) 冒充客户.",
                        _workContext.OriginalCustomerIfImpersonated.Email, _workContext.OriginalCustomerIfImpersonated.Id),
            ...
            if (orderPlacedStoreOwnerNotificationQueuedEmailId > 0)
            {
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("\"已下订单\" 发送给店主的电子邮件已在排队。排队的电子邮件标识符: {0}.", orderPlacedStoreOwnerNotificationQueuedEmailId),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
            ...
            if (orderPlacedCustomerNotificationQueuedEmailId > 0)
            {
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("\"已下订单\" 发送给客户的电子邮件已在排队. 排队的电子邮件标识符: {0}.", orderPlacedCustomerNotificationQueuedEmailId),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
            ...
            foreach (var vendor in vendors)
            {
                ...
                if (orderPlacedVendorNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("\"已下订单\" 发送给供应商的电子邮件已在排队. 排队的电子邮件标识符: {0}.", orderPlacedVendorNotificationQueuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
            ...
            order.OrderNotes.Add(new OrderNote
            {
                ...
                Note = string.Format("订单状态已更改为 {0}", os.ToString()),
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });
            _orderService.UpdateOrder(order);
                ...
                && notifyCustomer)
            {
                ...
                if (orderCompletedCustomerNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("\"订单已完成\" 电子邮件（给客户）已排队等候。 排队的电子邮件标识符: {0}.", orderCompletedCustomerNotificationQueuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
                ...
                && notifyCustomer)
            {
                ...
                if (orderCancelledCustomerNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("\"订单已取消\" 电子邮件（给客户）已排队等候。 排队的电子邮件标识符: {0}.", orderCancelledCustomerNotificationQueuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            if (processPaymentRequest == null)
                ...
                throw new ArgumentNullException("处理付款请求");
            ...
            if (!result.Success)
            {
                ...
                var logError = result.Errors.Aggregate("下订单时出错. ",
                    (current, next) => string.Format("{0}错误 {1}: {2}. ", current, result.Errors.IndexOf(next) + 1, next));
        ...
        public virtual void ProcessNextRecurringPayment(RecurringPayment recurringPayment, ProcessPaymentResult paymentResult = null)
        {
            ...
            try
            {
                ...
                if (!nextPaymentDate.HasValue)
                    ...
                    throw new NopException("下一个付款日期无法计算");
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    if (paymentMethod == null)
                        //throw new NopException("Payment method couldn't be loaded");
                        throw new NopException("付款方式无法加载");

                    if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
                        //throw new NopException("Payment method is not active");
                        throw new NopException("付款方式未启用");
                    ...
                    switch (_paymentService.GetRecurringPaymentType(processPaymentRequest.PaymentMethodSystemName))
                    {
                        case RecurringPaymentType.NotSupported:
                            //throw new NopException("Recurring payments are not supported by selected payment method");
                            throw new NopException("所选付款方式不支持定期付款");
                        case RecurringPaymentType.Manual:
                            processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                            break;
                        case RecurringPaymentType.Automatic:
                        ...
                        default:
                            //throw new NopException("Not supported recurring payment type");
                            throw new NopException("不支持定期付款类型");
                    }
                }
                else
                ...
                if (processPaymentResult == null)
                    //throw new NopException("processPaymentResult is not available");
                    throw new NopException("支付过程的结果不可用");
                ...
                else
                {
                    ...
                    var logError = processPaymentResult.Errors.Aggregate("处理定期订单时出错. ",
                       (current, next) => string.Format("{0}错误 {1}: {2}. ", current, processPaymentResult.Errors.IndexOf(next) + 1, next));
                    _logger.Error(logError, customer: customer);
                }
            }
            catch (Exception exc)
            {
                ...
                _logger.Error(string.Format("处理定期订单时出错. {0}", exc.Message), exc);
                throw;
            }
        }
        ...
        public virtual IList<string> CancelRecurringPayment(RecurringPayment recurringPayment)
        {
            ...
            if (initialOrder == null)
                ...
                return new List<string> { "初始订单无法加载" };
            ...
            catch (Exception exc)
            {
                ...
                result.AddError(string.Format("错误: {0}.完整异常: {1}", exc.Message, exc.ToString()));
            }
            ...
            for (int i = 0; i < result.Errors.Count; i++)
            {
                ...
                error += string.Format("错误 {0}: {1}", i, result.Errors[i]);
            ...
            if (!String.IsNullOrEmpty(error))
            {
                ...
                initialOrder.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("无法取消定期付款. {0}", error),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(initialOrder);
                ...
                string logError = string.Format("取消定期付款时出错. 订单 #{0}. 错误: {1}", initialOrder.Id, error);
        ...
        public virtual void Ship(Shipment shipment, bool notifyCustomer)
        {
            if (shipment == null)
                ...
                throw new ArgumentNullException("配送");

            var order = _orderService.GetOrderById(shipment.OrderId);
            if (order == null)
                //throw new Exception("Order cannot be loaded");
                throw new Exception("订单无法加载");

            if (shipment.ShippedDateUtc.HasValue)
                //throw new Exception("This shipment is already shipped");
                throw new Exception("此货已发货");
            ...
            order.OrderNotes.Add(new OrderNote
            {
                ...
                Note = string.Format("配送# {0} 已经发送", shipment.Id),
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });
            _orderService.UpdateOrder(order);
            ...
            if (notifyCustomer)
            {
                ...
                if (queuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("\"配送\"电子邮件（给客户）已排队等候。 排队的电子邮件标识符: {0}.", queuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual void Deliver(Shipment shipment, bool notifyCustomer)
        {
            ...
            order.OrderNotes.Add(new OrderNote
            {
                ...
                Note = string.Format("配送# {0} 已经送达", shipment.Id),
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });
            _orderService.UpdateOrder(order);
            ...
            if (notifyCustomer)
            {
                ...
                if (queuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("\"交付\" 电子邮件（给客户）已排队等候。 排队的电子邮件标识符: {0}.", queuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            catch (Exception exc)
            {
                ...
                //result.AddError(string.Format("Error: {0}. Full exception: {1}", exc.Message, exc.ToString()));
                result.AddError(string.Format("错误: {0}. 全异常 {1}", exc.Message, exc.ToString()));
            }
            ...
            if (!String.IsNullOrEmpty(error))
            {
                ...
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("捕获顺序错误. {0}", error),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
                ...
                //log it
                string logError = string.Format("捕获顺序错误.#{0}. 错误: {1}", order.Id, error);
                _logger.InsertLog(LogLevel.Error, logError, logError);
            }
            return result.Errors;
        }
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                ...
                throw new NopException("您不能将此订单标记为已付款");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("订单已退款， 金额 = {0}", request.AmountToRefund),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
                    ...
                    if (orderRefundedStoreOwnerNotificationQueuedEmailId > 0)
                    {
                        order.OrderNotes.Add(new OrderNote
                        {
                            ...
                            Note = string.Format("\"订单退款\"电邮（给店主）已排队。 排队的电子邮件标识符: {0}.", orderRefundedStoreOwnerNotificationQueuedEmailId),
                            DisplayToCustomer = false,
                            CreatedOnUtc = DateTime.UtcNow
                        });
                        _orderService.UpdateOrder(order);
                    ...
                    if (orderRefundedCustomerNotificationQueuedEmailId > 0)
                    {
                        order.OrderNotes.Add(new OrderNote
                        {
                            ...
                            Note = string.Format("\"订单退款\" 电子邮件（给客户）已排队等候。 排队的电子邮件标识符: {0}.", orderRefundedCustomerNotificationQueuedEmailId),
                            DisplayToCustomer = false,
                            CreatedOnUtc = DateTime.UtcNow
                        });
                        _orderService.UpdateOrder(order);
            ...
            catch (Exception exc)
            {
                ...
                //result.AddError(string.Format("Error: {0}. Full exception: {1}", exc.Message, exc.ToString()));
                result.AddError(string.Format("错误: {0}. 完全异常: {1}", exc.Message, exc.ToString()));
            ...
            if (!String.IsNullOrEmpty(error))
            {
                ...
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("无法退款的订单. {0}", error),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
                ...
                string logError = string.Format("退款订单错误 #{0}. 错误: {1}", order.Id, error);
                _logger.InsertLog(LogLevel.Error, logError, logError);
            }
            return result.Errors;
        }
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                ...
                throw new NopException("此订单不能退货");
            ...
            order.OrderNotes.Add(new OrderNote
            {
                ...
                Note = string.Format("订单已被标记为已退款， 金额 = {0}", amountToRefund),
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });
            _orderService.UpdateOrder(order);
            ...
            if (orderRefundedStoreOwnerNotificationQueuedEmailId > 0)
            {
                ...
                {
                    //Note = string.Format("\"Order refunded\" email (to store owner) has been queued. Queued email identifier: {0}.", orderRefundedStoreOwnerNotificationQueuedEmailId),
                    Note = string.Format("\"订单退款\" 电邮（给店主）已排队。 排队电子邮件标识符： {0}.", orderRefundedStoreOwnerNotificationQueuedEmailId),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    order.OrderNotes.Add(new OrderNote
                    {
                        ...
                        Note = string.Format("订单已部分退款， 金额 = {0}", amountToRefund),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
            ...
            if (!String.IsNullOrEmpty(error))
            {
                ...
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("无法部分退款. {0}", error),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
                ...
                //log it
                //string logError = string.Format("Error refunding order #{0}. Error: {1}", order.Id, error);
                string logError = string.Format("错误退款订单 #{0}. 错误: {1}", order.Id, error);
                _logger.InsertLog(LogLevel.Error, logError, logError);
            }
            return result.Errors;
        }
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                ...
                throw new NopException("这个订单你不能部分退款（离线）");
            ...
            order.OrderNotes.Add(new OrderNote
            {
                ...
                Note = string.Format("订单已被部分退款，金额  = {0}", amountToRefund),
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });
            _orderService.UpdateOrder(order);
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!String.IsNullOrEmpty(error))
            {
                ...
                order.OrderNotes.Add(new OrderNote
                {
                    ...
                    Note = string.Format("不能取消订单. {0}", error),
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
                ...
                string logError = string.Format("取消错订单错误 #{0}. 错误: {1}", order.Id, error);
                _logger.InsertLog(LogLevel.Error, logError, logError);
            }
            return result.Errors;
        }
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new NopException("你不能取消这个订单");
        ...
        public virtual bool ValidateMinOrderSubtotalAmount(IList<ShoppingCartItem> cart)
        {
            if (cart == null)
                ...
                throw new ArgumentNullException("车");
